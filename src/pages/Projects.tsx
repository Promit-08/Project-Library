            ...
            <div className="space-y-6">
              <div className="relative group w-full max-w-2xl">
                ...
                <input 
                  ...
                  className={cn(
                    "w-full border rounded-2xl pl-11 pr-24 sm:pr-28 py-3 sm:py-4 outline-none focus:ring-2 focus:ring-teal-500/50 transition-all shadow-xl font-sans text-sm sm:text-base",
                    theme === 'dark' 
                ...
                <div className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2">
                  <Button 
                    variant="ghost" 
                    size="sm"
                    className="rounded-xl bg-teal-500/10 text-teal-500 hover:bg-teal-500 hover:text-white px-3 sm:px-5 h-8 sm:h-9 text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-all"
                  >
                    Search
                  </Button>
                </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-none w-full items-center">
                  {categories.map(cat => (
                    <button
                      key={cat}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row relative">
        ...
        <div className={cn(
          "lg:w-80 h-56 lg:h-auto relative flex items-center justify-center overflow-hidden shrink-0 border-r",
          ...
          <div className="absolute top-6 left-6">
            <Badge className="bg-teal-500/10 text-teal-400 border border-teal-500/20 backdrop-blur-md py-1.5 px-4 rounded-full text-[10px] font-bold uppercase tracking-wider">
              {project.category}
            </Badge>
          </div>
        </div>

        <div className="flex-1 p-5 sm:p-7 lg:p-10 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className={cn("h-px w-4 sm:w-8 transition-colors", theme === 'dark' ? "bg-white/10" : "bg-slate-200")} />
              <div className="flex items-center gap-1.5">
                <User className="w-3 h-3 text-teal-500/50" />
                <span className={cn("text-[10px] uppercase tracking-wider font-bold", theme === 'dark' ? "text-teal-500" : "text-teal-600")}>Verified Scholar</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1">
              <div className={cn("flex items-center gap-2 sm:gap-4 p-1 rounded-full border transition-colors", theme === 'dark' ? "bg-white/5 border-white/5" : "bg-slate-50 border-slate-200")}>
                <button 
                  ...
                  className={cn(
                    "flex items-center gap-2 transition-all p-2 px-3 sm:px-4 rounded-full",
                    project.userInteraction === 'like' 
                      ? "bg-teal-500 text-white shadow-lg shadow-teal-500/20" 
                      : (theme === 'dark' ? "text-slate-400 hover:text-white" : "text-slate-500 hover:text-teal-600")
                  )}
                >
                  <ThumbsUp className={cn("w-3.5 h-3.5", project.userInteraction === 'like' && "fill-white/20")} />
                  <span className="text-[11px] font-bold leading-none">{project.likes}</span>
                </button>
                <button 
                ...
                >
                  <ThumbsDown className={cn("w-3.5 h-3.5", project.userInteraction === 'dislike' && "fill-white/20")} />
                  <span className="text-[11px] font-bold leading-none">{project.dislikes}</span>
                </button>
              </div>
            </div>

            <div className={cn(
              "flex items-center gap-2 px-4 py-2 rounded-full border transition-all group/stat",
              theme === 'dark' ? "bg-white/5 border-white/5 hover:border-white/10" : "bg-slate-50 border-slate-200 hover:bg-slate-100"
            )}>
              <MessageSquare className="w-3.5 h-3.5 text-slate-500 group-hover/stat:text-teal-500 transition-colors" />
              <span className={cn("text-[11px] font-bold transition-colors", theme === 'dark' ? "text-slate-400" : "text-slate-600")}>
                {project.comments_count} <span className="hidden sm:inline font-light opacity-50 ml-1">Insights</span>
              </span>
            </div>

            <div className={cn("flex flex-col gap-1 sm:pr-6 sm:border-r transition-colors", theme === 'dark' ? "border-white/10" : "border-slate-200")}>
