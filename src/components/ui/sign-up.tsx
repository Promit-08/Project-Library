...
// Importing animation components from motion/react
...
// const ConfettiContext = createContext<Api>({} as Api) // Unused in provided snippet

const Confetti = React.forwardRef<ConfettiRef, React.ComponentPropsWithRef<"canvas"> & { options?: ConfettiOptions; globalOptions?: ConfettiGlobalOptions; manualstart?: boolean }>((props, ref) => {
  ...
  const fire = React.useCallback((opts = {}) => instanceRef.current?.({ ...options, ...opts }), [options])
  const api = React.useMemo(() => ({ fire }), [fire])
  React.useImperativeHandle(ref, () => api, [api])
  React.useEffect(() => { if (!manualstart) fire() }, [manualstart, fire])
...
interface BlurFadeProps { children: React.ReactNode; className?: string; variant?: { hidden: { y: number }; visible: { y: number } }; duration?: number; delay?: number; yOffset?: number; inView?: boolean; inViewMargin?: string; blur?: string; wrapperKey?: string | number; key?: React.Key; }
function BlurFade({ children, className, variant, duration = 0.4, delay = 0, yOffset = 6, inView = true, inViewMargin = "-50px", blur = "6px", wrapperKey }: BlurFadeProps) {
  ...
  return (
    <motion.div key={wrapperKey} ref={ref} initial="hidden" animate={isInView ? "visible" : "hidden"} exit="hidden" variants={combinedVariants} transition={{ delay: 0.04 + delay, duration, ease: "easeOut" }} className={className}>
      {children}
    </motion.div>
  );
}
...
const GradientBackground = () => (
    <>
        ...
        <svg width="100%" height="100%" viewBox="0 0 800 600" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" className="absolute top-0 left-0 w-full h-full">
            <defs>
                <linearGradient id="rev_grad1" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'var(--color-primary, #3b82f6)', stopOpacity:0.8}} /><stop offset="100%" style={{stopColor: 'var(--color-chart-3, #ef4444)', stopOpacity:0.6}} /></linearGradient>
                <linearGradient id="rev_grad2" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" style={{stopColor: 'var(--color-chart-4, #f59e0b)', stopOpacity:0.9}} /><stop offset="50%" style={{stopColor: 'var(--color-secondary, #64748b)', stopOpacity:0.7}} /><stop offset="100%" style={{stopColor: 'var(--color-chart-1, #10b981)', stopOpacity:0.6}} /></linearGradient>
                <radialGradient id="rev_grad3" cx="50%" cy="50%" r="50%"><stop offset="0%" style={{stopColor: 'var(--color-destructive, #ef4444)', stopOpacity:0.8}} /><stop offset="100%" style={{stopColor: 'var(--color-chart-5, #8b5cf6)', stopOpacity:0.4}} /></radialGradient>
            ...
            <g style={{ animation: 'float2 25s ease-in-out infinite' }}>
                ...
                <ellipse cx="50" cy="150" rx="180" ry="120" fill="var(--color-accent, #e2e8f0)" filter="url(#rev_blur2)" opacity="0.8"/>
            </g>
        </svg>
    </>
);
...
export const AuthComponent = ({ 
  ...
  const [modalStatus, setModalStatus] = React.useState<'closed' | 'loading' | 'error' | 'success'>('closed');
  ...
  const handleFinalSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLoginFlow && password !== confirmPassword) {
    ...
    try {
      ...
      setTimeout(() => {
          ...
          // Wait a bit more for the success animation before calling onSuccess
          setTimeout(() => {
  ...
  const handleProgressStep = () => {
    if (authStep === 'email') {
        ...
        if (isPasswordValid) {
          if (isLoginFlow) {
            // In login flow, we submit after password
    ...
    <AnimatePresence>
        {modalStatus !== 'closed' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-slate-900 border-2 border-white/20 rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-6 mx-4 shadow-2xl">
                    {(modalStatus === 'error' || modalStatus === 'success') && <button onClick={closeModal} className="absolute top-4 right-4 p-1 text-white/50 hover:text-white transition-colors"><X className="w-6 h-6" /></button>}
                    {modalStatus === 'error' && <>
                        <div className="p-4 rounded-full bg-rose-500/20 border border-rose-500/50">
                            <AlertCircle className="w-10 h-10 text-rose-500" />
                        </div>
                        <p className="text-xl font-bold text-white text-center leading-tight">{modalErrorMessage}</p>
                        <GlassButton onClick={closeModal} size="sm" className="mt-2" contentClassName="text-white font-bold">Try Again</GlassButton>
                    </>}
                    {modalStatus === 'loading' && 
                        <TextLoop interval={TEXT_LOOP_INTERVAL} stopOnEnd={true}>
                            {modalSteps.slice(0, -1).map((step, i) => 
                                <div key={i} className="flex flex-col items-center gap-6">
                                    ...
                                    <p className="text-xl font-bold text-white tracking-tight">{step.message}</p>
                                </div>
                            )}
                        </TextLoop>
                    }
  ...
  return (
    <div className="bg-background min-h-screen w-screen flex flex-col overflow-hidden">
        ...
        <motion.div 
            ...
            className={cn( 
              "fixed top-8 left-8 z-20 flex items-center gap-4", 
              "md:left-1/2 md:-translate-x-1/2" 
            )}
        >
            <div className="scale-125 md:scale-150">{logo}</div>
            <h1 className="text-3xl md:text-5xl font-serif italic text-white drop-shadow-md tracking-tight">{brandName}</h1>
        </motion.div>
            ...
            <fieldset disabled={modalStatus !== 'closed'} className="relative z-10 flex flex-col items-center gap-8 w-[280px] mx-auto p-4">
                <AnimatePresence mode="wait">
                    {authStep === "email" && <motion.div key="email-content" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center gap-4">
                        <BlurFade delay={0.25 * 1} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl md:text-6xl tracking-tight text-white whitespace-nowrap">{isLoginFlow ? 'Welcome Back' : 'Get started with Us'}</p></div></BlurFade>
                    </motion.div>}
                    {authStep === "password" && <motion.div key="password-title" initial={{ y: 6, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full flex flex-col items-center text-center gap-4">
                        <BlurFade delay={0} className="w-full"><div className="text-center"><p className="font-serif font-light text-4xl sm:text-5xl tracking-tight text-white whitespace-nowrap">{isLoginFlow ? 'Enter your password' : 'Create your password'}</p></div></BlurFade>
                        <BlurFade delay={0.25 * 1}><p className="text-sm font-medium text-white/70">{isLoginFlow ? 'Secure your session to continue.' : 'Your password must be at least 6 characters long.'}</p></BlurFade>
                    </motion.div>}
                ...
                <form id="auth-form" onSubmit={handleFinalSubmit} className="w-[300px] space-y-6">
                     <AnimatePresence>
                        {authStep !== 'confirmPassword' && <motion.div key="email-password-fields" exit={{ opacity: 0, filter: 'blur(4px)' }} transition={{ duration: 0.3, ease: "easeOut" }} className="w-full space-y-6">
                            <BlurFade delay={authStep === 'email' ? 0.25 * 5 : 0} inView={true} className="w-full">
                                <div className="relative w-full">
                                    <AnimatePresence>
                                        {authStep === "password" && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3, delay: 0.4 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/50 font-semibold">Email</label></motion.div>}
                                    </AnimatePresence>
                            ...
                            <AnimatePresence>
                                {authStep === "password" && <BlurFade key="password-field" wrapperKey="password-field" className="w-full">
                                    <div className="relative w-full">
                                        <AnimatePresence>
                                            {password.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/50 font-semibold">Password</label></motion.div>}
                                        </AnimatePresence>
                                        <div className="glass-input-wrap w-full"><div className="glass-input">
                                            <span className="glass-input-text-area"></span>
                                            ...
                                            <input ref={passwordInputRef} type={showPassword ? "text" : "password"} placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={handleKeyDown} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/40 focus:outline-none" />
                                    ...
                                    <BlurFade inView delay={0.2}><button type="button" onClick={handleGoBack} className="mt-4 flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors"><ArrowLeft className="w-4 h-4" /> Go back</button></BlurFade>
                                </BlurFade>}
                            </AnimatePresence>
                        </motion.div>}
                    </AnimatePresence>
                    <AnimatePresence>
                        {authStep === 'confirmPassword' && <BlurFade key="confirm-password-field" wrapperKey="confirm-password-field" className="w-full">
                            <div className="relative w-full">
                                <AnimatePresence>
                                    {confirmPassword.length > 0 && <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.3 }} className="absolute -top-6 left-4 z-10"><label className="text-xs text-white/50 font-semibold">Confirm Password</label></motion.div>}
                                </AnimatePresence>
                                <div className="glass-input-wrap w-[300px]"><div className="glass-input">
                                    ...
                                    <input ref={confirmPasswordInputRef} type={showConfirmPassword ? "text" : "password"} placeholder="Confirm Password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="relative z-10 h-full w-0 flex-grow bg-transparent text-white placeholder:text-white/40 focus:outline-none" />
                ...
                <BlurFade delay={0.5} className="mt-4">
                  <button 
                    ...
                    className="text-xs font-bold text-white/50 hover:text-white transition-colors tracking-widest uppercase"
                  >
